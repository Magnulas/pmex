...
wasserstein_distance(const Diagram& dgm1, const Diagram& dgm2, unsigned p)
{
    ...
    for (unsigned i = 0; i < dgm1.size(); ++i)
        for (unsigned j = 0; j < dgm2.size(); ++j)
        {
            ...
            m(i,j) = std::pow((double)(norm(p1, p2)),  (double)p);
    ...
    for (unsigned i = 0; i < dgm1.size(); ++i)
        ...
        {
            const Point& p1 = *(dgm1.begin() + i);
            m(i,j) = std::pow((double)(norm.diagonal(p1)), (double)p);
        }
    ...
    for (unsigned j = 0; j < dgm2.size(); ++j)
        ...
        {
            const Point& p2 = *(dgm2.begin() + j);
            m(i,j) = std::pow((double)(norm.diagonal(p2)),(double) p);
        }
        ...
        for (unsigned j = 0; j < size; j++)
            if (m(i,j) == 0)
            {
                ...
                if (i >= dgm1.size())
                {
                    ...
                    {
                        const Point& p2 = *(dgm2.begin() + j);
                        sum += std::pow((double)(norm.diagonal(p2)), (double)p);
                    }
                } else
                {
                    if (j >= dgm2.size())
                    {
                        const Point& p1 = *(dgm1.begin() + i);
                        sum += std::pow((double)(norm.diagonal(p1)),(double) p);
                    } else
                    {
                        const Point& p1 = *(dgm1.begin() + i);
                        const Point& p2 = *(dgm2.begin() + j);
                        sum += std::pow((double)(norm(p1, p2)), (double) p);
                    }
                }
                break;
            }
